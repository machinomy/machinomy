 ...
 * @param callback
 */
Client.prototype.handlePaymentRequired = function (uri, response, callback) {
    ...
    if (version == VERSION) {
        ...
        var value = response.headers['paywall-price'];
            ...
            if (docs.length == 0) {
                ...
                self.contract.buildPaymentChannel(self.sender, receiver, value, function (error, paymentChannel) {
                    ...
                    self.storage.saveChannel(paymentChannel, function (error) {
                        if (error) throw error;
                        var payment = channel.Payment.fromPaymentChannel(paymentChannel, value);
                        self.transport.requestToken(gateway, payment, function (error, token) {
                            self.transport.getWithToken(uri, token, function (error, response) {
...
Server.prototype.acceptPayment = function (payment, callback) {
    ...
    if (payment.recipient != self.receiver) {
        throw ("Recipient must be " + self.receiver);
    }

    self.storage.channelsBySenderReceiverChannelId(payment.sender, payment.recipient, payment.channelId, function (err, docs) {
        if (docs.length == 0) {
