...
const web3 = new Web3() // FIXME Extract as a parameter
...
const buildPaymentChannel = (contract, sender, receiver, value, callback) => {
  ...
  let settlementPeriod = ChannelContract.SETTLEMENT_PERIOD
  let duration = ChannelContract.DURATION
  let options = {
    from: sender,
    value: value,
...
class ChannelContract {
  ...
  claim (receiver, channelId, value, v, r, s, callback) {
    let h = this.h(channelId, value)
    this.contract.claim(channelId, value, h, parseInt(v), r, s, {from: receiver})
    let didSettle = this.contract.DidSettle({channelId: channelId})
  ...
  startSettle (account, channelId, payment, callback) {
    let self = this
    ...
    let didStartSettleEvent = self.contract.DidStartSettle({channelId: channelId, payment: payment})
  ...
  finishSettle (account, channelId, callback) {
    let self = this
...
const CONTRACT_ADDRESS = configuration.contractAddress()
const CONTRACT_INTERFACE = configuration.contractInterface()
const CONTRACT = new ChannelContract(CONTRACT_ADDRESS, CONTRACT_INTERFACE)
