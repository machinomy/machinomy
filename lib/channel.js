...
ChannelContract.prototype.claim = function (receiver, channelId, value, v, r, s, callback) {
    ...
    console.log(r);
    console.log(s);
    self.contract.claim(channelId, value, parseInt(v), r, s, {from: receiver});
    var didClaimEvent = self.contract.DidClaim({channelId: channelId});
    didClaimEvent.watch(function (error, result) {
        console.log("Claimed " + result.args.value + " from " + result.args.channelId);
        didClaimEvent.stopWatching();
        callback(error, result.args.value);
    });
};
...
var close = function (paymentChannel, callback) {
    console.log("Trying to close PaymentChannel " + paymentChannel.channelId);
