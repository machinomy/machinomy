...
class Sender {
  ...
  freshChannel (uri, response, paymentRequired, valueCallback) {
    ...
    valueCallback(null, paymentRequired.price, (error, value, callback) => { // Determine value of the channel
      ...
      this.contract.buildPaymentChannel(this.sender, paymentRequired.receiver, value, (error, paymentChannel) => {
        if (error) throw error
  ...
  existingChannel (uri, response, paymentRequired, raw, valueCallback) {
    ...
    valueCallback(null, paymentRequired.price, (error, value, callback) => {
      if (error) throw error
      let paymentChannel = new channel.PaymentChannel(raw.sender, raw.receiver, raw.channelId, null, raw.value, raw.spent)
      let payment = channel.Payment.fromPaymentChannel(paymentChannel, paymentRequired.price)
      let nextPaymentChannel = channel.PaymentChannel.fromPayment(payment)
  ...
  handlePaymentRequired (uri, response, valueCallback) {
    ...
    if (version === VERSION) {
      ...
      channels.allByQuery({sender: this.sender, receiver: paymentRequired.receiver}).then(docs => {
        let openChannels = []
        for (let doc of docs) {
        ...
        if (openChannels.length === 0) {
          ...
          // Do Something
          throw new Error(`Found more than one channel from ${this.sender} to ${paymentRequired.receiver}`)
        }
      })
    } else {
      // Do Something
  ...
  pry (uri, callback) {
    this.transport.get(uri, {}, (error, response) => {
      if (error) {
        callback(error)
      } else {
