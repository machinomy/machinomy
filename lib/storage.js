...
class Storage {
   ...
   */
  constructor (path, namespace, inMemoryOnly) {
     ...
     */
    this.channels = channels(storageEngine, namespace)
  ...
  firstById (channelId) {
    ...
    return this.engine.findOne(query).then(document => {
      ...
      if (document) {
        ...
        let state = 0 //channel.contract.getState(channelId.toString()) // FIXME
        result = new channel.PaymentChannel(document.sender, document.receiver, document.channelId, document.value, document.spent, state)
      } else {
  ...
  allByQuery (q) {
    ...
    return this.engine.find(query).then(found => {
      ...
      return _.map(found, doc => {
        let state = 0 // channel.contract.getState(doc.channelId.toString()) // FIXME
 ...
 * @returns {Storage}
 */
const build = (path, namespace = null, inMemoryOnly = false) => {
  return new Storage(path, namespace, inMemoryOnly)
}
