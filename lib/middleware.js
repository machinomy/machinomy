 ...
 * @constructor
 */
var Paywall = function (web3, account, address, _storage) {
  var settings = configuration.receiver()
  log.debug('Use settings for receiver', settings)
  _storage = _storage || storage.build(web3, settings.databaseFile, 'receiver')
  this.receiverAccount = account
  this.gatewayUri = urljoin(address, configuration.PAYWALL_PATH)
  this.server = receiver.build(web3, account, _storage)
}
...
Paywall.prototype.middleware = function () {
  ...
  var handler = function (req, res) {
    ...
    self.server.acceptPayment(payment).then(token => {
      res.status(202)
