...
export namespace Storage {
  export function build (databaseUrl: string, channelContract: ChannelContract, inflator: Inflator): Promise<Storage> {
    ...
    switch (protocol) {
      case 'nedb':
        return buildNedb(splits[1], channelContract, inflator, namespace)
      case 'postgresql':
        return buildPostgres(databaseUrl, channelContract, inflator, namespace)
      case 'sqlite':
        return buildSqlite(databaseUrl, channelContract, inflator, namespace)
      default:
  ...
  async function buildNedb (databaseUrl: string, channelContract: ChannelContract, inflator: Inflator, namespace: string): Promise<Storage> {
    ...
    return {
      ...
      channelsDatabase: new NedbChannelsDatabase(engine, channelContract, inflator, namespace),
      migrator: undefined
    }
  }
  ...
  async function buildSqlite (databaseUrl: string, channelContract: ChannelContract, inflator: Inflator, namespace: string): Promise<Storage> {
    ...
    return {
      ...
      channelsDatabase: new SqliteChannelsDatabase(engine, channelContract, inflator, namespace),
  ...
  async function buildPostgres (databaseUrl: string, channelContract: ChannelContract, inflator: Inflator, namespace: string): Promise<Storage> {
    ...
    return {
      ...
      channelsDatabase: new PostgresChannelsDatabase(engine, channelContract, inflator, namespace),
